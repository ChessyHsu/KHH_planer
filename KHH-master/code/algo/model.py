...
class CHARGE_TYPE(IntEnum):
    """self.Types of tugs which are used in charging and planning.
...
class TASK():
    ...
    def __init__(self, i, ship, tug_cnt, ship_state, start_time, start, dest, side=SIDE.OPTIONAL):
        self.id = i
        self.ship = ship
    ...
    def __str__(self):
        return("TASK {}: ship {}\n* Depart at: {}\n* From {} to {}\n* Goes: {}\n* Side: {}".format(
            ...
            self.start, self.to, self.ship_state.name, self.side.name))

    def show(self):
        res = "======== Task {} Result ========\n".format(self.id)
        res += "* Ship ID: {}\n* Started at: {}\n* Working time: {} mins\n* State: {}\n* Weight: {} \n\n".format(
            self.ship.ship_id, self.start_time.strftime("%Y-%m-%d %H:%M"), self.work_time.seconds//60, self.task_state.name, self.ship.weight)
        res += "* Tug Set :"
        for t in self.tugs:
            res += "{} ".format(t.tug_id)
        print(res)
